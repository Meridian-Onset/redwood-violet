...
class Ensemble:
    ...
    def __init__(self, actor_type, num_actors, reward_type=Instance, reward_scarcity=0.5, day_night=True):
        ...
        for i in range(num_actors):
            self.players.append(actor_type(i, self.field_size))
    ...
    def toArray(self, objects_type: str) -> List[npt.ArrayLike]:
        ...
        if objects_type.lower() in ('actors', 'agents'):
            ...
            raise InvalidObjectError(f"Not a valid object type, use one of the following: \n {valids}")
        return[x, y]

    def start(self, cycles: int, buildVideo: bool = False) -> None:
    ...
    def Display_Config(self, save: bool = False, *args, **kwargs) -> None:
        ...
        if save:
            plt.savefig(f'Snapshots/{input("Enter destination filename")}.png')
        plt.show()
...
class Dependent_Food_Scarcity_Ensemble(Ensemble):
    # TODO: Write this class that has the food scarcity change depending on the amount of food leftover
    # TODO: the previous day
    def __init__(self):
        super(Dependent_Food_Scarcity_Ensemble, self).__init__(Basic_Actor, 100)
